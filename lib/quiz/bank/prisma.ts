...
export const prismaQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    topic: "prisma",
    difficulty: "easy",
    prompt: "`@unique` prevents two records from storing the same non-null value for that field.",
    correct: true,
    explanation: "`@unique` enforces uniqueness for non-null values; on PostgreSQL, multiple NULLs are allowed for optional unique fields.",
  },
  {
    id: 251,
    type: "boolean",
    topic: "prisma",
    difficulty: "medium",
    prompt: "`findMany` returns an array (not a single object), even when its `where` clause filters by a unique field.",
    correct: true,
    explanation: "`findMany` always resolves to an array (0 or 1 elements for a unique filter); use `findUnique` when you want a single object or null.",
  },
  {
    id: 252,
    type: "boolean",
  ...
  },
  {
    ...
    topic: "prisma",
    difficulty: "medium",
    prompt: "Order the usual Prisma 7 steps after adding a model field that needs a tracked development migration:",
    items: [
      { id: "schema", label: "Update schema.prisma" },
      { id: "migrate", label: "Run prisma migrate dev" },
      { id: "generate", label: "Run prisma generate" },
      { id: "client", label: "Use the updated generated Client" },
    ],
    correctOrder: ["schema", "migrate", "generate", "client"],
    explanation: "Define the schema change, apply it with migrate dev, explicitly run prisma generate (no longer automatic in Prisma 7), then use the updated Client.",
  },
  {
    id: 257,
    type: "order",
